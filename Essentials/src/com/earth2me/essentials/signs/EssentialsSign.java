...
public class EssentialsSign {
    ...
    protected final Trade getTrade(final ISign sign, final int index, final int decrement, final IEssentials ess) throws SignException {
        ...
        if (money == null) {
            ...
            if (item.equalsIgnoreCase("times")) {
                ...
                sign.updateSign();
                return new Trade(signName.toLowerCase(Locale.ENGLISH) + "sign", ess, sign);
            } else if (item.equalsIgnoreCase("exp") || item.equalsIgnoreCase("xp")) {
                sign.setLine(index, quantity + " exp");
                return new Trade(quantity, ess, sign);
            } else {
                final ItemStack stack = getItemStack(item, quantity, ess);
                sign.setLine(index, quantity + " " + item);
                return new Trade(stack, ess, sign);
            }
        } else {
            return new Trade(money, ess, sign);
        }
    }
    ...
    static class EventSign implements ISign {
		...
		public final SignType getType() {
			return SignType.valueOf(getLine(0).toUpperCase().replace("[", "").replace("]", ""));
		}
